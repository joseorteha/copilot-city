...
import { getPublicRepository, getRawFile, getRepositoryTree, GitHubApiError } from "@/lib/github/github-client";
...
export async function POST(request: Request) {
  try {
    ...
    if (!payload.success) return NextResponse.json({ error: "Pega una URL válida de GitHub." }, { status: 400 });
    ...
    if (!result.filesIncluded) return NextResponse.json({ error: "No encontramos archivos textuales exportables." }, { status: 422 });
    ...
    if (error instanceof RepositoryUrlError) return NextResponse.json({ error: error.message }, { status: 400 });
    if (error instanceof GitHubApiError) return NextResponse.json({ error: error.message }, { status: error.status >= 500 ? 502 : error.status });
